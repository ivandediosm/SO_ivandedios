            ...
            while (true)
            {
                ...
                switch (codigo)
                {
                    case 1:  // respuesta a longitud
                        if (trozos.Length > 2) // Verifica que hay suficientes elementos
                        {
                    ...
                    case 3:  // Recibimos la respuesta de si soy alto
                        ...
                        break;
                    case 4:  // Recibimos notificación si el nombre es palíndromo
                        if (trozos.Length > 2) // Verifica que hay suficientes elementos
                        {
                            nform = Convert.ToInt32(trozos[1]);
                            mensaje = trozos[2].Split('\0')[0];
                        ...
                        break;
                    case 5:  // Respuesta para convertir a mayúsculas
                        if (trozos.Length > 2) // Verifica que hay suficientes elementos
                        {
                            nform = Convert.ToInt32(trozos[1]);
                            mensaje = trozos[2].Split('\0')[0];
